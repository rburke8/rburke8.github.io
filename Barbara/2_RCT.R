...
descr(nsw, stats = c("mean", "sd", "min", "med", "max"), transpose = TRUE)

freq(nsw$treated, report.nas = FALSE)
...
pstest_scatter(data=result, xmax=25)
...
# Compare beta in the following 3 regressions:
...
# Compare SE(beta) in the following 3 regressions:

summ(rct_mod0, digits = 3)

rct_mod3 <- lm(re78 ~ treated + age + age2 + educ + nodegree, data = nsw)
summ(rct_mod3, digits = 3)

rct_mod4 <- lm(re78 ~ treated + re75 + black, data = nsw)
summ(rct_mod4, digits = 3)
...
summ(lm(re78 ~ treated + treated_old + old + age + age2 + educ + black + hispanic + re75, data = nsw), digits = 3)
