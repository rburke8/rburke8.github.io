...
summ(lm(re78 ~ treated + age + age2 + black + hispanic + married, 
          data = nsw_psid), robust = "HC1", digits = 3)

summ(lm(re78 ~ treated + educ + nodegree, 
          data = nsw_psid), robust = "HC1", digits = 3)

summ(lm(re78 ~ treated + re75, 
          data = nsw_psid), robust = "HC1", digits = 3)

summ(lm(re78 ~ treated + age + age2 + black + hispanic + married + educ + nodegree + re75, 
          data = nsw_psid), robust = "HC1", digits = 3) 
...
ols_int <- lm(re78 ~ treated 
        ...
        + age + age2 + black + hispanic + married + educ + nodegree + re75, 
        data = nsw_psid)
